...
def get_loader(mode, split_index, dataset='summe'):
    if mode.lower() == 'train':
        vd = VideoData(mode, split_index, dataset)
        return DataLoader(vd, batch_size=1)
    else:
        return VideoData(mode, split_index, dataset)


def get_difference_attention(dataset):
    if dataset == 'summe':
        return open_pickle_file('summe_diff_attention.pickle')
    elif dataset == 'tvsum':
